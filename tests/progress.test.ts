...
test('an unplayed game reports a clean slate', () => {
  assert.deepEqual(progressFor(EMPTY_PROGRESS, 'memory'), { rounds: 0, stars: 0, bestLevel: 1 });
});
...
test('rounds and stars accumulate per game without touching the others', () => {
  ...
  assert.deepEqual(progressFor(progress, 'memory'), { rounds: 2, stars: 4, bestLevel: 2 });
  assert.deepEqual(progressFor(progress, 'counting'), { rounds: 1, stars: 2, bestLevel: 1 });
